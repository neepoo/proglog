...
type StreamLayer struct {
	in net.Listener
	//we need to take in the TLS configs used
	//to accept incoming connections (the serverTLSConfig)
...
func NewStreamLayer(in net.Listener, serverTLSConfig *tls.Config, peerTLSConfig *tls.Config) *StreamLayer {
	return &StreamLayer{in: in, serverTLSConfig: serverTLSConfig, peerTLSConfig: peerTLSConfig}
}
...
// setupLog(dataDir string)
// creates the log for this server, where this server
//will store the user’s records.
...
func (l *DistributedLog) setupRaft(dataDir string) error {
	...
	// 比如一个新server假如cluster,那么不需要从leader获取所有的历史数据,直接从快照恢复会高效得多
	...
	// The config’s LocalID field is the unique ID for this server
...
func (l *DistributedLog) apply(reqType RequestType, req proto.Message) (interface{}, error) {
	...
	timeout := 10 * time.Second
	/*
		The l.raft.Apply(buf.Bytes(), timeout) call has a lot going on
		behind the scenes, running the steps described in Log Replication, to
