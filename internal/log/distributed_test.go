...
func TestMulNode(t *testing.T) {
	...
	for i := 0; i < nodeCount; i++ {
		...
		require.NoError(t, err)
		config := Config{}
		config.Raft.StreamLayer = NewStreamLayer(in, nil, nil)
		...
		require.NoError(t, err)
		if i != 0 {
			err = logs[0].Join(fmt.Sprintf("%d", i), in.Addr().String())
			require.NoError(t, err)
		} else {
	...
	for _, record := range records {
		...
		require.Eventually(t, func() bool {
			for j := 0; j < nodeCount; j++ {
				...
				record.Offset = off
				if !reflect.DeepEqual(got.Values, record.Values) {
					return false
				}
			}
			return true
	...
	off, err := logs[0].Append(&api.Record{Values: []byte("third")})
	require.NoError(t, err)
	...
	require.NoError(t, err)
	require.Equal(t, []byte("third"), record.Values)
